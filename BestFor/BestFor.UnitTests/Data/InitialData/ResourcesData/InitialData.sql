...
if not exists(select * from ResourceString where CultureName = 'en-US' and [Key] = 'contact_us')
	insert ResourceString(CultureName, [Key], Value, DateAdded) values('en-US', 'contact_us',
		N'Contact Us', getDate());
if not exists(select * from ResourceString where CultureName = 'ru-RU' and [Key] = 'contact_us')
	insert ResourceString(CultureName, [Key], Value, DateAdded) values('ru-RU', 'contact_us',
		N'клщыывдал', getDate());
GO

if not exists(select * from ResourceString where CultureName = 'en-US' and [Key] = 'suggestions_upper')
	insert ResourceString(CultureName, [Key], Value, DateAdded) values('en-US', 'suggestions_upper',
		N'Suggestions', getDate());
if not exists(select * from ResourceString where CultureName = 'ru-RU' and [Key] = 'suggestions_upper')
	insert ResourceString(CultureName, [Key], Value, DateAdded) values('ru-RU', 'suggestions_upper',
		N'ываываыав', getDate());
GO

if not exists(select * from ResourceString where CultureName = 'en-US' and [Key] = 'contact_us_page_text')
	insert ResourceString(CultureName, [Key], Value, DateAdded) values('en-US', 'contact_us_page_text',
		N'We are always glad to hear from our users! Let us know how we can help you!', getDate());
if not exists(select * from ResourceString where CultureName = 'ru-RU' and [Key] = 'contact_us_page_text')
	insert ResourceString(CultureName, [Key], Value, DateAdded) values('ru-RU', 'contact_us_page_text',
		N'ываываыав', getDate());
GO
